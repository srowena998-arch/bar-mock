...
test('Eval Suite: 5 Standard Benchmarks Defined', () => {
  assert.equal(EVAL_TEST_CASES.length, 5);
...
test('API GET /api/evals/test-cases: Returns all standard test cases', async () => {
  ...
  assert.equal(res.json.test_cases.length, 5);
});
...
test('API POST /api/evals/run-single: Executes vector chunk diet benchmark', async () => {
  ...
  assert.ok(res.json.result.metrics.chunk_token_count < 800);
...
test('API POST /api/evals/run-all: Executes full throttled benchmark suite', async () => {
  ...
  assert.equal(res.status, 200);
  assert.equal(res.json.success, true);
  assert.equal(res.json.scorecard.total_tests, 5);
  assert.equal(res.json.scorecard.passed_count, 5);
